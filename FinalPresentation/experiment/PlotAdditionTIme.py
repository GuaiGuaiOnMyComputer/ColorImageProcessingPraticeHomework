...
def load_txt_stats() -> np.ndarray:
    ...
    for (root, dirs, files) in walk(join("FinalPresentation", "experiment", "log", "additiontime")):
    ...
    np.save(join("FinalPresentation", "experiment", "log", "additiontime", "stats_addition_time.npy"), stats)
    return stats
...
def plot_curve(data):
    ...
    plt.ylabel("Addition Time (ms)")
    plt.title("Mat Addition Time of 7 Image Shapes")
    plt.show()
